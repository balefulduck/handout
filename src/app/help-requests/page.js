...
export default function UserHelpRequestsPage() {
  ...
  const fetchUserHelpRequests = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/help-requests/user');
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <Link 
              href="/plants"
              className="inline-flex items-center px-3 py-1.5 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-olive-green"
            >
              Zurück zu Meine Pflanzen
            </Link>
          </div>
                  ...
                  >
                    ...
                    {request.admin_response && (
                      <div className="mt-2 bg-green-50 p-2 rounded-md">
                        <p className="text-sm text-gray-700 font-medium">Antwort von Dr. Cannabis:</p>
                        <p className="text-sm text-gray-600 line-clamp-2">{request.admin_response}</p>
                      </div>
                    )}
                    <div className="mt-2 flex items-center text-xs text-gray-500">
                      ...
                      {JSON.parse(request.file_urls || '[]').length > 0 && (
                        <span className="flex items-center">
                          ...
                          {JSON.parse(request.file_urls || '[]').length} Bild(er) angehängt
                        </span>
                      )}
                    </div>
                  </div>
